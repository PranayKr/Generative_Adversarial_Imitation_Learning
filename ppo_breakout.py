...
class PPOTrain:
    ...
    def train(self, obs, actions, gaes, rewards, v_preds_next):
        tf.get_default_session().run(self.train_op, feed_dict={self.Policy.input_normalized: obs,
                                                               self.Old_Policy.input_normalized: obs,
    ...
    def get_summary(self, obs, actions, gaes, rewards, v_preds_next):
        return tf.get_default_session().run(self.merged, feed_dict={self.Policy.input_normalized: obs,
                                                                    self.Old_Policy.input_normalized: obs,
                                                                    self.actions: actions,
                                                                    self.rewards: rewards,
                                                                    self.v_preds_next: v_preds_next,
    ...
    def get_gaes(self, rewards, v_preds, v_preds_next):
        ...
        # calculate generative advantage estimator(lambda = 1)
        gaes = copy.deepcopy(deltas)
        for t in reversed(range(len(gaes) - 1)):  # is T-1, where T is time step which runS policy
            gaes[t] = gaes[t] + self.gamma * gaes[t + 1]
        return gaes

    def get_grad(self, obs, actions, gaes, rewards, v_preds_next):
        return tf.get_default_session().run(self.gradients, feed_dict={self.Policy.input_normalized: obs,
                                                                       self.Old_Policy.input_normalized: obs,
                                                                       self.actions: actions,
                                                                       self.rewards: rewards,
                                                                       self.v_preds_next: v_preds_next,
